...
if __name__ == '__main__':
    ...
    for channel in channels_all:
        ...
        for item in res_1['items']:
            lst_1 = get_video_stats(item['snippet']['resourceId']['videoId'])
            ...
            for temp in lst_1["items"]:
                ...
                cemp.setdefault('videoId', temp['id'])
                cemp.setdefault('videoTitle', videoTitle)
                cemp.setdefault("videoPublishedAt", videoPublishedAt)
                cemp.setdefault('viewCount', temp['statistics']['viewCount'])
                cemp.setdefault('likeCount', temp['statistics']['likeCount'])
                cemp.setdefault('dislikeCount', temp['statistics']['dislikeCount'])
                cemp.setdefault('commentCount', temp['statistics']['commentCount'])
    ...
    for item in res_2['items']:
        lst_2 = get_video_stats(item['id']['videoId'])
        videoTitle = item['snippet']['title']
        videoPublishedAt = item["snippet"]["publishedAt"].encode("ascii", "ignore")
    ...
    for item in res_3['items']:
        lst_3 = get_video_stats(item['id']['videoId'])
        videoTitle = item['snippet']['title']
        videoPublishedAt = item["snippet"]["publishedAt"].encode("ascii", "ignore")
    ...
    for item in res_4['items']:
        lst_4 = get_video_stats(item['id']['videoId'])
        videoTitle = item['snippet']['title']
        videoPublishedAt = item["snippet"]["publishedAt"].encode("ascii", "ignore")
    ...
    for item in res_5['items']:
        lst_5 = get_video_stats(item['id']['videoId'])
        videoTitle = item['snippet']['title']
        videoPublishedAt = item["snippet"]["publishedAt"].encode("ascii", "ignore")
    ...
    with open("YouTube_videoStats_all.csv", "w", newline='', encoding='utf-8') as comment_file:
