...
if __name__ == '__main__':
    ...
    min_view = stats.nsmallest(10, 'viewCount')
    join_df = pd.merge(min_view, NLP, on='videoId', how='left')
    join_df_ = join_df[np.isfinite(join_df['commentCount'])]

    result = []
    title = []
    for row in min_view['videoId']:
        line = []
    ...
    plt.title("Min_View: Top 20 most common words")
    plt.show()
