...
if __name__ == '__main__':
    ...
    min_like = stats[stats.commentCount != 0].nsmallest(10, 'likeCount')

    join_df = pd.merge(min_like, NLP, on='videoId', how='left')
    join_df_ = join_df[np.isfinite(join_df['commentCount'])]
    # print(join_df_)
    result = []
    title = []
    for row in min_like['videoId']:
        line = []
    ...
    plt.title("Min_Like: Top 20 most common words")
    plt.show()
