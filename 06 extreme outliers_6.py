...
if __name__ == '__main__':
    ...
    min_dislike = stats.nsmallest(10, 'dislikeCount')

    join_df = pd.merge(min_dislike, NLP, on='videoId', how='left')
    join_df_ = join_df[np.isfinite(join_df['commentCount'])]
    # print(join_df_)
    result = []
    title = []
    for row in min_dislike['videoId']:
        line = []
    ...
    plt.title("Min_Dislike: Top 20 most common words")
    plt.show()
